...
import { clearHistory, getHistory, saveCurrentResult } from '@/lib/pachinko/store';
import { RetroButton, RetroMetric, RetroPage, RetroPanel } from '@/components/ui/Retro';
import type { HistoryEntry } from '@/types/pachinko';
    ...
    >
      <div className="min-w-0">
        ...
        <p className="retro-label mt-1">
          {formatDate(entry.timestamp)} / 評価 {entry.estimatedSetting.toFixed(1)} / ボーダー比率 {entry.estimatedPayback.toFixed(1)}%
        </p>
      </div>
      ...
      <RetroPanel title="計算履歴">
        ...
        {history.length > 0 ? (
          ...
          <div className="py-12 text-center">
            ...
            <div className="mt-5">
              <RetroButton onClick={() => router.push('/pachinko/calculation')}>入力画面へ</RetroButton>
            </div>
          </div>
        )}
      </RetroPanel>
