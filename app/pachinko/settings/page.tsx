...
import { getSettings, saveSettings } from '@/lib/pachinko/store';
import {
  RetroButton,
  RetroDataRow,
  RetroInput,
  ...
  return (
    <RetroPage
      ...
      commands={[
        ...
        { href: '/pachinko/calculation', label: '入力' },
      ]}
      message="設定値は端末内に保存されます。貸し玉、交換率、通常回転速度を実戦環境に合わせて調整してください。"
    >
      <div className="grid gap-3 lg:grid-cols-2">
        <RetroPanel title="ホール設定">
          <RetroDataRow label="貸し玉" description="1玉あたりの金額（円）">
            <NumberSetting value={settings.ballRate} onChange={(v) => update('ballRate', v)} min={1} max={10} step={0.1} label="貸し玉" />
          </RetroDataRow>
          <RetroDataRow label="交換率" description="100円あたりの交換玉数。28玉交換なら28">
            <NumberSetting value={settings.exchangeRate} onChange={(v) => update('exchangeRate', v)} min={20} max={50} step={0.1} label="交換率" />
          </RetroDataRow>
          <RetroDataRow label="持ち玉優先" description="持ち玉がある場合に優先使用">
            <Toggle checked={settings.useCashBalls} onChange={(v) => update('useCashBalls', v)} />
          </RetroDataRow>
        </RetroPanel>

        <RetroPanel title="計算設定">
          <RetroDataRow label="1時間あたりの通常回転数" description="実戦ペースに合わせて調整">
            <NumberSetting value={settings.startsPerHour} onChange={(v) => update('startsPerHour', v)} min={120} max={360} step={10} label="1時間あたりの通常回転数" />
          </RetroDataRow>
      ...
      <RetroPanel title="回転率評価モデル">
        ...
        <div className="retro-box-grid mt-4">
          <RetroMetric label="貸し玉" value={`${settings.ballRate}`} unit="円" />
          <RetroMetric label="交換率" value={`${settings.exchangeRate}`} unit="玉" />
          <RetroMetric label="通常速度" value={`${settings.startsPerHour}`} unit="回/h" />
      ...
      <RetroPanel title="アプリ情報">
        <p className="retro-value text-lg">PACHINKO COMPASS</p>
        <p className="retro-label mt-2 leading-relaxed">
          個人利用を目的とした期待値計算ツールです。掲載スペックは参照元ページから取り込んだ確認用データであり、実際の出玉を保証するものではありません。
        </p>
      </RetroPanel>
    </RetroPage>
  );
